...
let customHandlers = {};
customHandlers["demo"] = function (instance, component, item, value) { }
...
class CustomAttributer extends csharp_1.FairyEditor.View.PluginInspector {
    ...
    customDataObj = {};
    customHandlers = {};
    constructor(data, customHandlers) {
        super();
        this.panel = csharp_1.FairyGUI.UIPackage.CreateObject("CustomAttributer", "Main").asCom;
        let { components, mode, pattern, parent, objectTypeLimit, componentTypeLimit } = data;
        this.customHandlers = customHandlers;
        this.components = components;
        this.pattern = pattern;
        this.objectTypeLimit = objectTypeLimit;
        this.componentTypeLimit = componentTypeLimit;
        this.parent = parent;
    ...
    updateUI() {
        ...
        if (this.objectTypeLimit != null)
            pattern = pattern && this.objectTypeLimit.includes(getObjectTypeName(inspectingTarget));
        if (this.componentTypeLimit != null)
            pattern = pattern && this.componentTypeLimit.includes(getComponentExtensionId(inspectingTarget));
    ...
    renderItem(component, item, reset) {
        ...
        let customHandler = this.customHandlers[key];
        if (customHandler != undefined) {
            customHandler(this, component, item, value);
            return;
        }

        this.renderCommonItem(component, item, value);
    }
    renderCommonItem(component, item, value) {
        // 下拉框
        if (component instanceof csharp_1.FairyGUI.GComboBox && item.type == index_1.EComponent.COMBOBOX) {
            ...
            component.value = itemArr.get_Item(value || 0);
        }
    ...
    getListItemVal() {
        for (let i = 0; i < this.list.numChildren; i++) {
            ...
            let defaultVal = this.components[i].value;
            if (this.customDataObj) {
                if (value == defaultVal) {
                    if (key in this.customDataObj) {
                        delete this.customDataObj[key];
                    }
                }
                else {
...
let getObjectTypeName = (gObject) => {
    if (gObject instanceof csharp_1.FairyEditor.FComponent) {
        if (gObject.extention != null) {
            return gObject.extention._type;
        }
    }

    return gObject.objectType;
};
let getComponentExtensionId = (gObject) => {
    if (gObject instanceof csharp_1.FairyEditor.FComponent) {
        if (gObject._res != null && gObject._res.packageItem != null) {
            return gObject._res.packageItem.GetURL();
        }
    }
...
for (let i = 0; i < config.inspectors.length; i++) {
    ...
    App.inspectorView.AddInspector(() => new CustomAttributer(inspector, customHandlers), title, title);
